...
export class EiaAdapter implements Adapter {
  public readonly id = "eia";
  ...
  public async fetch(config: AdapterConfig): Promise<Result<AdapterFetchResult>> {
    try {
      const payloadData = { crude_stocks_barrels: 425000000 };
  ...
  public parse(rawPayload: any): Result<readonly Observation[]> {
    try {
      const obs: Observation = {
        ...
        pillar: this.pillar,
        entity_id: null,
        metric_key: "ENERGY_CRUDE_OIL_INVENTORY_BARRELS",
        value: 425000000n as ScaledInteger,
