...
export class ManifoldAdapter implements Adapter {
  public readonly id = "manifold";
  ...
  public async fetch(config: AdapterConfig): Promise<Result<AdapterFetchResult>> {
    try {
      const payloadData = { market_slug: "ai-agi-2027", probability: 0.42 };
  ...
  public parse(rawPayload: any): Result<readonly Observation[]> {
    try {
      const obs: Observation = {
        ...
        pillar: this.pillar,
        entity_id: null,
        metric_key: "MANIFOLD_PROBABILITY_PCT",
        value: 42n as ScaledInteger,
