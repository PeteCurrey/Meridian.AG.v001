...
export class CoinGeckoAdapter implements Adapter {
  ...
  public async fetch(config: AdapterConfig): Promise<Result<AdapterFetchResult>> {
    try {
      const payloadData = { bitcoin: { usd: 67450.00 } };
  ...
  public parse(rawPayload: any): Result<readonly Observation[]> {
    try {
      const obs: Observation = {
        ...
        pillar: this.pillar,
        entity_id: null,
        // Canonical Metric Mapping: Dual-Source Example #3 with Twelve Data BTC/USD
        metric_key: "CRYPTO_BTC_PRICE_USD",
        value: 6745000n as ScaledInteger, // $67,450.00
