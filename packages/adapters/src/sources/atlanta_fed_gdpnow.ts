...
export class AtlantaFedGdpNowAdapter implements Adapter {
  public readonly id = "atlanta_fed_gdpnow";
  ...
  public async fetch(config: AdapterConfig): Promise<Result<AdapterFetchResult>> {
    try {
      const payloadData = { gdpnow_estimate_pct: 2.7 };
  ...
  public parse(rawPayload: any): Result<readonly Observation[]> {
    try {
      const obs: Observation = {
        ...
        pillar: this.pillar,
        entity_id: null,
        // Canonical Metric Mapping: Dual-Source Example #1 with FRED_GDP
        metric_key: "MACRO_US_GDP",
        value: 270n as ScaledInteger,
